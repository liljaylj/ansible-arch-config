...
measure_download() {
    while read -r server; do
        ...
        read -r startt speed < <(curl -m 10 -sLw '%{time_starttransfer} %{speed_download}\n' -o /dev/null "$base_url/chaotic-aur.files.tar.zst")
        echo "$startt $speed $server"
    done
}

curl -m 10 -sL 'https://raw.githubusercontent.com/chaotic-aur/pkgbuild-chaotic-mirrorlist/main/mirrorlist' |
